...
| A-08 | High | Governance/control defect | Implemented, follow-up proposed | Actions now has a read-only, secret-free pull-request test workflow, plus concurrency protection, a 90-minute screener timeout, fail-fast offline tests, immutable action pins, non-publishing manual validation runs, and retained validation artifacts. Follow-up: pin Python dependencies and combine result/snapshot publication into one push. |
